...
function Remote_Asterisk($asterisk,$number,$state)
{
	...
	//check response call
		...
		$pamiClient->registerEventListener(
			...
			{
				echo $event->DialStatus();
				$Active=1;	
				switch($event->DialStatus())
				{
					case 'INVALIDARGS'://no extension or number
							
						break;
					case 'NOANSWER'://no answer or timeout
						 if($GLOBALS['state']===3)
        		                                $Active=0;
			Write_Database($GLOBALS['host'],$GLOBALS['user'],$GLOBALS['pass'],$GLOBALS['database'],$GLOBALS['table'],$GLOBALS['state'],'TIMEOUT',$GLOBALS['number'],$Active);
				...
				Write_Database($GLOBALS['host'],$GLOBALS['user'],$GLOBALS['pass'],$GLOBALS['database'],$GLOBALS['table'],$GLOBALS['state'],'BUSY',$GLOBALS['number'],$Active);
						break;
					case 'CHANUNAVAIL'://congested or not available
						  if($GLOBALS['state']==3)
                				        $Active=0;
			...
			{
				if($event instanceof  DialEvent&& $event->SubEvent=='Begin')
				{
					return true;
				}
			}
